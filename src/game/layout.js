...
export function createInitialRecSlots() {
  const arr = [];
  for (let i = 0; i < 8; i++) arr.push({ x: 620, y: 70 + i * 96, w: 320, h: 86, idx: i });
  return arr;
}

export function createInitialCommentSlots() {
  const arr = [];
...
// Spawns the final suspicious comment hiding the hole
export function createPropaganda(slot) {
  return [{
...
// Spawns the truth (hole) underneath
export function createTruth(slot) {
